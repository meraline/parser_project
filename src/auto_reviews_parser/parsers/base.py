...
from ..utils.metrics import ParserMetrics
...
class BaseParser(ABC):
    ...
    def random_delay(self):
        ...
        delay = random.uniform(
            self.delay_manager.min_delay, self.delay_manager.max_delay
        )
        time.sleep(delay)
...
class BaseParser(ABC):
    """Base parser providing shared utilities."""
    ...
    def apply_delay(self, min_delay: float = 0, max_delay: float = 0) -> None:
        """Применить задержку между запросами."""
        self.delay_manager.delay(min_delay, max_delay)

    def extract_common_fields(self, text: str) -> Dict[str, Any]:
        """Извлечь общие поля из текста отзыва."""
        try:
